...
void cliRegisterTable(char *name, char *help, int numberInTable, CliJte *table)
{
    ...
    {
        printf("cli", "Out of space in command table.\n" );
        return;
    }

    if (cliFindCommand(name, &cliRootNode) != NULL)
    {
        printf("cli", "Table '%s' registered twice.\n", name );
        return;
    }
...
static Bool cliGetStringAndTokenise( char *string, CliJte *node )
{
    ...
    if ( tokenCount > 0 )
    {
        ...
        {
            printf("cli", "Unrecognised command, try '?' for help\n" );
        }
    }
    ...
    if ((command = cliFindCommand(tokens[0], jte)) != NULL)
    {
        ...
        if (command->numberInNextLevel > 0)
        {
            ...
            if (tokenCount == 1)
            {
				...
				printf("cli", "Type '.' to exit, or '?' for local help\n");
        ...
        else if (command->callback != NULL)
        {
            ...
            {
				printf("cli", "Not enough arguments; at least %d required.\n", (-command->argCount) - 1);
            }
            else if ((command->argCount > 0) && (tokenCount != command->argCount))
            {
				printf("cli", "Incorrect number of arguments.\n");
            }
            else
            {
                /* Call callback. */
...
static void cliHelp (CliJte* jte)
{
    ...
    if (jte != &cliRootNode)
    {
        ...
        {
            printf("cli", "%-25s", jte->name);
        }
        else
#endif
        {
            printf("cli", "%-14s", jte->name);
        }
